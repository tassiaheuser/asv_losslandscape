...
def set_weights(net, weights, directions=None, step=None):
    ...
    if directions is None:
        ...
        # # debug stuff
        # names = []
        # for n,npx in net.named_parameters():
        #     names.append(n)
        # i = 0

        for (p, w, d) in zip(net.parameters(), weights, changes):
            # print(names[i])
            # if names[i] == '__S__.fc.bias':
            #     print("wooow")
            # i += 1
            if type(d) is np.float64: additive = torch.Tensor([d]).type(type(w))
            else: additive = torch.Tensor(d).type(type(w))
            p.data = w + additive
...
def setup_direction(args, dir_file, net):
    ...
    if not args.dir_file:
        ...
        if args.model_file2:
            net2 = model_loader_toplevel.load(args.dataset, args.model, args.model_file2)
        ...
        if args.y:
            if args.same_dir:
                ...
                net3 = model_loader_toplevel.load(args.dataset, args.model, args.model_file3)
...
def name_direction_file(args):
    ...
    if file2:
        ...
        rm_file_ending = file1[:file1.rfind(".")]
        if "/" in rm_file_ending:
            dir_file += rm_file_ending[rm_file_ending.rfind("/"):].replace("/", "")
        else:
            dir_file += rm_file_ending
    ...
    return os.path.join(args.out_dir, dir_file)
