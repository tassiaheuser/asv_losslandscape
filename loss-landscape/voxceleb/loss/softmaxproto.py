...
import voxceleb.loss.softmax as softmax
import voxceleb.loss.angleproto as angleproto
...
class LossFunction(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        nlossS, prec1   = self.softmax(x.reshape(-1,x.size()[-1]), label.repeat_interleave(x.size()[1]))
