...
from voxceleb.model.ResNetBlocks_6s8g import *
import voxceleb.model.ResNetBlocks as Rnb
...
class ResNetSE(nn.Module):
    def __init__(self, block, layers, num_filters, nOut, encoder_type='SAP', n_mels=80, log_input=True, **kwargs):
        ...
        self.layer0_1 = Res2NeXt(96, 96, kernel_size=3, scale=6)
        self.layer0_2 = Res2NeXt(96, 96, kernel_size=3, scale=6)
        self.layer0_3 = Res2NeXt(96, 96, kernel_size=3, scale=6)
        ...
        if self.encoder_type == "SAP":
            self.sap_linear = nn.Linear(num_filters[3], num_filters[3])
            self.attention = self.new_parameter(num_filters[3], 1)
...
def MainModel(nOut=256, **kwargs):
    # Number of filters
    num_filters = [96, 96, 96, 96]
    model = ResNetSE(SEBottleneck, [3, 4, 6, 3], num_filters, nOut, **kwargs)
    #model = ResNetSE(Rnb.SEBasicBlock, [3, 4, 6, 3], num_filters, nOut, **kwargs)
    return model
