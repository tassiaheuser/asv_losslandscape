...
def project_1D(w, d):
    ...
    #scale = torch.dot(w, d)/d.norm()/d.norm() #!!!!!
    d = d / d.norm()
    scale = torch.dot(w, d)
    return scale.item()
...
def project_trajectory(dir_file, w, s, model_files,
               proj_method='cos', dir_type='weights', **kwargs):
    """
    ...
    dx = nplist_to_tensor(directions[0]).to(torch.float32)
    dy = nplist_to_tensor(directions[1]).to(torch.float32)

    xcoord, ycoord = [], []
    for model_file in model_files:
        net2 = model_loader_toplevel.load(kwargs["dataset"], kwargs["model"], model_file=model_file, kwargs=kwargs)
        if dir_type == 'weights':
...
def setup_PCA_directions(args, model_files, w, s, **kwargs):
    """
    ...
    for model_file in model_files:
        print (model_file)
        # net2 = model_loader_toplevel.load(args.dataset, args.model, model_file)
        net2 = model_loader_toplevel.load(model_file=model_file,**kwargs)
