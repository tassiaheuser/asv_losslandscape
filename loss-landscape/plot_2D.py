...
def plot_2d_contour(surf_file, surf_name='train_loss', vmin=0.1, vmax=10, vlevel=0.5, zmax=0, show=False,circle=False,log_scale_plot=False,normalise=False):
    ...
    # cut the square area to a circle by replacing the Z values with nan
    if circle:
        radius = np.min([np.max(x), np.max(y)])
        for i in range(len(x)):
            for j in range(len(y)):
                if np.sqrt((x[i] - np.mean(x))**2 + (y[j] - np.mean(y))**2) > radius:
                    Z[i, j] = np.nan
    ...
    fig.savefig(surf_file_no_fileending + '_' + surf_name + '_2dcontour' + '.pdf', dpi=300,
                bbox_inches='tight', format='pdf')

    fig = plt.figure()
    print(surf_file + '_' + surf_name + '_2dcontourf' + '.pdf')
    CS = plt.contourf(X, Y, Z, cmap='summer', levels=np.arange(vmin, vmax, vlevel))
    fig.savefig(surf_file_no_fileending + '_' + surf_name + '_2dcontourf' + '.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
    ...
    sns_plot.get_figure().savefig(surf_file_no_fileending + '_' + surf_name + '_2dheat.pdf',
    ...
    if log_scale_plot:
        ...
        Z = np.log(Z + 1)  # add a small value to avoid log(0)
        surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    if normalise:
        Z = (Z - Z[~np.isnan(Z)].mean()) / Z[~np.isnan(Z)].std()  # normalize the surface
        surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)   
    else:
        surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
...
def plot_trajectory(proj_file, dir_file, show=False):
    ...
    fig = plt.figure()
    plt.plot(f['proj_xcoord'][:], f['proj_ycoord'][:], marker='.') #? added [:]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--zmax', default=0.1, type=float, help='Miminum value to map')
    ...
    if exists(args.surf_file) and exists(args.proj_file) and exists(args.dir_file):
        ...
        plot_2d_contour(args.surf_file, args.surf_name, args.vmin, args.vmax, args.vlevel, args.zmax, args.show)
