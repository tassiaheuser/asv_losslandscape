...
class SEBottleneck(nn.Module):
    ...
    #def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=12):
...
class Res2NeXt(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, scale=6):
        super(Res2NeXt, self).__init__()
        width = int(math.floor(planes / 6))
        self.conv1 = nn.Conv2d(inplanes, width * 6, kernel_size=1)
        self.bn1 = nn.BatchNorm2d(width * 6)
        self.nums = 6 - 1
        convs = []
        bns = []
        num_pad = math.floor(3 / 2) 
        for i in range(self.nums):
            convs.append(nn.Conv2d(width, width, kernel_size=3, groups=8, padding=num_pad))
            bns.append(nn.BatchNorm2d(width))
        self.convs = nn.ModuleList(convs)
        self.bns = nn.ModuleList(bns)
        self.conv3 = nn.Conv2d(width * 6, planes, kernel_size=1)
